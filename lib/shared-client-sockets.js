...
module.exports = function configureSharedSockets(cfg) {
  ...
  function recieveNotificationKey (details, finish) {
    ...
    return rk8pki.decrypt( details.notificationInfo, clientPki.privateKey, function (err, result) {
        ...
        try {
          notificationInfo = JSON.parse(result);
        } catch (e) {
        ...
        clientPki.timeout = setTimeout( function(){
          ...
          // which will delete the entry from the pkiMap unless it already has a
          // notificationPublicKey.
          completeRegistration({ identity: details.identity }, function noop(){});
        }, 15000);
        /*
        ...
        */
        ...
        return rk8pki.encrypt(
          ...
          done // recieves (error, encryptedResponse)
        );
    });
  ...
  function completeRegistration (details, finish) {
    ...
    */
    ...
    return rk8pki.decrypt( details.credentials, clientPki.privateKey, function (err, result) {
        var credentials;
  ...
  function acknowledgeSend(details, socketInfo, err, acknowledement){
    ...
    persistentPkiMap.get({id: socketInfo.identity}, function(err, clientPki){
      ...
      return rk8pki.decrypt(acknowledement, clientPki.privateKey, function (err, ack) {
          if (err) {
              return callAckFn(err);
          }
